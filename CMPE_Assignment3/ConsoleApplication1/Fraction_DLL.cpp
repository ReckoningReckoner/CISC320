...
Fraction::Fraction(int numerator, int denominator) {
	...
	if (denominator == 0) {
		throw FractionException("Divide by zero encountered.");
	}

	bool eitherIsNegative = numerator < 0 || denominator < 0;
	bool bothAreNegative = numerator < 0 && denominator < 0;
	const int sign = eitherIsNegative && !bothAreNegative ? -1 : 1;
	...
	while (buffer[i] != '\0') {
		...
		if ('0' <= c && c <= '9') {
			...
			*n += charAsDigit;
			...
			if (n == &denominator) {
				throw FractionException("There can only be one '/' symbol at a time.");
			}
			...
			throw FractionException("Invalid character encountered.");
		}
		i++;
	}
